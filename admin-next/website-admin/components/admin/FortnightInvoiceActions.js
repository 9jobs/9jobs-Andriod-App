...
export default function FortnightInvoiceActions({
  invoiceId,
  hasGeneratedPdf,
  initialCheckoutUrl = '',
  initialPaymentLinkSentAt = '',
  initialAutopayStatus = 'not_applicable',
  initialStripeSubscriptionId = '',
  initialPaymentMode = 'upfront',
}) {
  ...
  const [autopayStatus, setAutopayStatus] = useState(initialAutopayStatus);
  ...
  async function runAction(action, url, options = {}) {
    ...
    try {
      ...
      if (data.invoice?.autopayStatus) {
        setAutopayStatus(data.invoice.autopayStatus);
      } else if (data.paymentMode === 'monthly_autopay') {
        setAutopayStatus('pending_checkout');
      } else if (data.paymentMode === 'upfront') {
        setAutopayStatus('not_applicable');
      }

      if (data.invoice?.stripeSubscriptionId) {
  ...
  return (
    <div className="admin-actions-row">
      <button
        ...
        onClick={() => runAction('generate', `/admin/website-admin/api/fortnight-invoices/${invoiceId}/generate-pdf`)}
        type="button"
      >
        {pendingAction === 'generate' ? 'Generating...' : hasGeneratedPdf ? 'Regenerate Preview' : 'Generate Preview'}
      </button>
      <Link className="admin-dark-button admin-dark-button--link" href={`/admin/website-admin/fortnight-invoices/${invoiceId}/edit`} prefetch={false}>
        Edit
      </Link>
      <button
        className="admin-dark-button"
        disabled={pendingAction === 'paymentLink'}
        onClick={() => runAction('paymentLink', `/admin/website-admin/api/fortnight-invoices/${invoiceId}/payment-link`)}
        type="button"
      >
      ...
      <button
        ...
        onClick={() => runAction('emailLink', `/admin/website-admin/api/fortnight-invoices/${invoiceId}/send-payment-link`)}
        type="button"
      >
      ...
      <button
        ...
        onClick={() => runAction('send', `/admin/website-admin/api/fortnight-invoices/send/${invoiceId}`)}
        type="button"
      >
      ...
      <button
        ...
        onClick={() =>
          runAction('whatsapp', `/admin/website-admin/api/fortnight-invoices/${invoiceId}/payment-link`, {
            onSuccess: (data) => {
      ...
      {checkoutUrl ? (
        <section className="admin-panel admin-payment-link-panel" aria-labelledby={`fortnight-payment-link-${invoiceId}`}>
          <div className="admin-panel__header admin-panel__header--compact">
            <div>
              <h2 id={`fortnight-payment-link-${invoiceId}`}>Payment Link</h2>
            </div>
          </div>
