...
const sections = [
  ...
  },
  {
    ...
    fields: ['monthLabel', 'issuedDate', 'validUntil', 'dueDate', 'description', 'duration', 'total'],
  },
];
...
export default function FortnightInvoiceForm({ initialValues = null, invoiceId = '', mode = 'create' }) {
  ...
  async function handleSubmit(event) {
    ...
    const validation = fortnightInvoiceInputSchema.safeParse(values);
    ...
    try {
      const response = await fetch('/admin/website-admin/api/fortnight-invoices', {
      ...
      pushToast({ title: mode === 'edit' ? 'Fortnight invoice updated.' : 'Fortnight invoice created.', tone: 'success' });
      startTransition(() => {
        router.push(`/admin/website-admin/fortnight-invoices/${data.invoice._id}`);
        router.refresh();
      });
    } finally {
      setIsPending(false);
    ...
    <form className="admin-form" onSubmit={handleSubmit}>
      <section className="admin-panel admin-panel--hero">
        <div className="admin-panel__header admin-panel__header--stack">
          <div>
            <h2>Fortnight Invoice Workspace</h2>
